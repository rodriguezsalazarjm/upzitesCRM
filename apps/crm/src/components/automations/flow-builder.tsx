...
function Editor(props: BuilderProps) {
  ...
  const field = (label: string, value: string | number, change: (v: string) => void, multiline = false) => <label className="block space-y-1 text-xs text-slate-600">{label}{multiline ? <textarea disabled={readonly} className={inputClass} rows={4} value={value} onChange={e => change(e.target.value)} /> : <input disabled={readonly} className={inputClass} value={value} onChange={e => change(e.target.value)} />}</label>;
  return <section className="space-y-4">
    ...
    <div className="grid gap-3 rounded-xl border bg-white p-4 sm:grid-cols-3"><label className="text-xs">Cuenta<select disabled={readonly} className={inputClass} value={trigger.accountId ?? ''} onChange={e => setTrigger({ ...trigger, accountId: e.target.value || undefined })}><option value="">Selecciona una cuenta</option>{props.accounts.filter(a => a.channel === props.channel).map(a => <option key={a.id} value={a.id}>{a.displayName || a.channel} · {a.status}</option>)}</select></label>
    ...
    <div className="grid min-h-[620px] gap-3 lg:grid-cols-[170px_minmax(0,1fr)_280px]">
      ...
      <aside className="space-y-3 rounded-xl border bg-white p-4"><h3 className="font-semibold">{current ? labels[current.type] : 'Configuración del nodo'}</h3>{!current && <p className="text-sm text-slate-500">Selecciona un nodo. Arrastra desde la paleta o pulsa para añadir. Une los conectores para definir el recorrido.</p>}
        {current?.type === 'MESSAGE' && <>{field('Texto', current.text, v => update({ text: v }), true)}<label className="text-xs">Canal: heredar {props.channel}<select className={inputClass} disabled={readonly} value={current.delivery ?? 'DM'} onChange={e => update({ delivery: e.target.value })}><option value="DM">Mensaje directo</option>{!missingCapabilities(account, ['PUBLIC_COMMENT_REPLY']).length && <option value="PUBLIC_REPLY">Respuesta pública a comentario</option>}{!missingCapabilities(account, ['PRIVATE_REPLY_TO_COMMENT']).length && <option value="PRIVATE_REPLY">Respuesta privada a comentario</option>}</select></label><p className="text-xs text-slate-500">Este nodo envía texto. Los adjuntos y botones aún no tienen implementación de envío en Flow.</p></>}
        ...
        {current?.type === 'AI' && <><label className="text-xs">Agente / versión publicada<select className={inputClass} disabled={readonly} value={current.agentVersionId ?? ''} onChange={e => update({ agentVersionId: e.target.value, allowedTools: props.agents.find(a => a.id === e.target.value)?.allowedTools ?? [] })}><option value="">Seleccionar</option>{props.agents.map(a => <option key={a.id} value={a.id}>{a.label}</option>)}</select></label>{field('Objetivo', current.goal, v => update({ goal: v }), true)}{field('Herramientas permitidas (coma)', current.allowedTools.join(', '), v => update({ allowedTools: v.split(',').map(t => t.trim()).filter(Boolean) }))}{field('Condiciones de salida (una por línea)', current.exitConditions.join('\n'), v => update({ exitConditions: v.split('\n').filter(Boolean) }), true)}{field('Máximo de pasos del agente', current.maxTurns, v => update({ maxTurns: Number(v) }))}</>}
